	...
	public void cargarDatos() throws FileNotFoundException {
		...
		while(scan.hasNextLine()) {
			...
			if(partes[2].toLowerCase().equals("refresco")) {
				if(partes[8].toLowerCase().equals("si")) {
					zumo=true;
				}
				if(partes[9].toLowerCase().equals("si")) {
					gaseoso=true;
				}
				Refresco refres=new Refresco(partes[0], partes[1], partes[2], partes[3],Integer.parseInt(partes[4]), Double.parseDouble(partes[5]), Integer.parseInt(partes[6]), partes[7], zumo, gaseoso,Integer.parseInt(partes[10]));
